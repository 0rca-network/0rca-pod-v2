...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen }) => {
  ...
  return (
    <aside className={`glass-effect w-64 flex-shrink-0 flex flex-col fixed left-0 top-0 bottom-0 z-30 transition-all duration-500 ease-in-out ${
        ...
        <nav className="px-4 pb-4 space-y-2">
          ...
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`nav-link flex items-center gap-3 px-4 py-2 rounded-lg text-gray-300 transition-colors hover:bg-white/5 ${
