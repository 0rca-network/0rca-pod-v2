...
const ConnectWalletModal = ({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm z-[99999] p-4" onClick={onClose}>
      <div
        className="bg-neutral-800 text-white rounded-lg shadow-xl w-full max-w-md p-6 m-auto relative"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center w-full mb-4">
          <h3 className="text-lg font-semibold text-white">Connect to a wallet</h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-2 w-full">
          {wallets.map((wallet) => (
            <div
              ...
              className={`flex justify-between items-center p-3 rounded-lg cursor-pointer transition-colors w-full
                ${
                  wallet.isConnected
                    ? "bg-mint-glow/20 border border-mint-glow hover:bg-mint-glow/30"
          ...
          {activeAccount && (
            <div
              ...
              className="flex justify-between items-center p-3 rounded-lg cursor-pointer transition-colors bg-red-900/50 border border-red-700 hover:bg-red-900/70 mt-4 w-full text-red-300"
            >
        ...
        <div className="mt-6 pt-4 border-t border-neutral-700 text-sm text-center text-neutral-400 w-full">
          <span>New to Algorand? </span>
          <a
            href="https://algorand.com/wallets"
            target="_blank"
            rel="noopener noreferrer"
            className="text-mint-glow hover:opacity-90"
          >
