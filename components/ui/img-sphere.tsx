...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const [wireframeDots, setWireframeDots] = useState<SphericalPosition[]>([]);
  ...
  const generateWireframeDots = useCallback((): SphericalPosition[] => {
    const dots: SphericalPosition[] = [];
    const goldenRatio = (1 + Math.sqrt(5)) / 2;
    const angleIncrement = 2 * Math.PI / goldenRatio;

    for (let i = 0; i < wireframeDotCount; i++) {
      const t = i / wireframeDotCount;
    ...
    return dots;
  }, [wireframeDotCount, actualSphereRadius]);
  ...
  useEffect(() => {
    ...
    if (showWireframe) {
      setWireframeDots(generateWireframeDots());
    }
  }, [generateSpherePositions, generateWireframeDots, showWireframe]);
  ...
  const calculateWireframePositions = useCallback((): WorldPosition[] => {
    return wireframeDots.map((pos, index) => {
      const thetaRad = SPHERE_MATH.degreesToRadians(pos.theta);
      const phiRad = SPHERE_MATH.degreesToRadians(pos.phi);
      const rotXRad = SPHERE_MATH.degreesToRadians(rotation.x);
      const rotYRad = SPHERE_MATH.degreesToRadians(rotation.y);
      ...
      const worldPos: Position3D = { x, y, z };
      const isVisible = worldPos.z > -50;
      const fadeOpacity = isVisible ? Math.max(0.2, Math.min(1, (worldPos.z + 50) / 100)) : 0;

      return {
        ...worldPos,
        scale: 1,
        zIndex: Math.round(500 + worldPos.z),
        isVisible,
        fadeOpacity,
        originalIndex: index
      };
    });
  }, [wireframeDots, rotation, actualSphereRadius]);

  const wireframePositions = showWireframe ? calculateWireframePositions() : [];
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    return (
      ...
      >
        ...
        {isHovered && (
          <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 px-3 py-2 bg-neutral-900 border border-mint-glow/30 rounded-lg shadow-xl whitespace-nowrap pointer-events-none z-50">
            <p className="text-white font-semibold text-sm">{image.title}</p>
            {image.description && (
              <p className="text-neutral-400 text-xs mt-1 max-w-xs truncate">{image.description}</p>
            )}
          </div>
        )}
      </div>
      ...
      >
        {showWireframe && (
          <div className="absolute w-full h-full" style={{ zIndex: 5 }}>
            {wireframePositions.map((pos, index) => {
